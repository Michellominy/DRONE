...
typedef struct {
    ...
    XHardware_pid_engine_uio_map maps[ MAX_UIO_MAPS ];
} XHardware_pid_engine_uio_info;

/***************** Variable Definitions **************************************/
static XHardware_pid_engine_uio_info uio_info;
...
static int uio_info_read_name(XHardware_pid_engine_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHardware_pid_engine_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHardware_pid_engine_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHardware_pid_engine_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHardware_pid_engine_Initialize(XHardware_pid_engine *InstancePtr, const char* InstanceName) {
	XHardware_pid_engine_uio_info *InfoPtr = &uio_info;
